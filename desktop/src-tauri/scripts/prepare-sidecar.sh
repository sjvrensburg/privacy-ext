#!/usr/bin/env bash
# Builds the pii-native-host sidecar and stages it where Tauri's `externalBin`
...
staged="binaries/pii-native-host-${triple}${ext}"

# tauri-build's build.rs checks that every externalBin path already exists
# before it lets ANYTHING in this package compile (including pii-native-host
# itself) — so stage a placeholder first to break the chicken-and-egg, then
# overwrite it with the real binary once built.
mkdir -p binaries
[ -f "$staged" ] || : > "$staged"

cargo build --release --bin pii-native-host

cp "target/release/pii-native-host${ext}" "$staged"
echo "Staged ${staged}"
